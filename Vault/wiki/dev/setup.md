...
GITHUB_APP_PRIVATE_KEY=[store the downloaded PEM contents in local .env only; never paste key material into wiki]
COPILOT_GITHUB_TOKEN=[store token in local .env only; never paste token material into wiki]
```
